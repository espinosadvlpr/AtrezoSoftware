...
function peticionTodosLosProductos() {
    return new Promise((resolve, reject) => {
        fetch('http://3.84.226.84:3050/product')
...
function cargarCategorias() {
    return new Promise((resolve, reject) => {
        fetch('http://3.84.226.84:3050/categorias')
            .then(response => response.json())
            .then(data => {
                console.log(data);
...
function eliminarProducto() {
    if (window.confirm("Realmente desea eliminar este producto?")) {
        var retrievedObject = localStorage.getItem('ProductoActual');
        var objetoProducto = JSON.parse(retrievedObject)
        fetch('http://3.84.226.84:3050/delete_product/' + objetoProducto.codigoProducto, {
            method: 'DELETE',
        })
...
function cargarProductosPorCategoria(idCategoria) {
    ...
    fetch('http://3.84.226.84:3050/product/' + idCategoria)
        .then(response => response.json())
        .then(data => {
